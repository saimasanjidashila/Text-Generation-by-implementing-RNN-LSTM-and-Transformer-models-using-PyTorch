...
def train_model():
    ...
    train_samples = full_dataset.samples[:50000]
    ...
    output = model.generate(tokenizer, prompt, eos_token_id=2, device=device)
...
def evaluate_on_test(model, tokenizer, test_file, device):
    ...
    with torch.no_grad():
        for input_ids, target_ids in tqdm(test_loader, desc="Test Eval"):
            ...
            #reference = nltk.word_tokenize(decoded_target)
